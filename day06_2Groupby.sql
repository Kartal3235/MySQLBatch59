...
select ulke,round(avg(maas),2) from personel group by ulke;
-- round yuvarlama yapıyor,2(virgülden sonra iki basamak ekliyor)

-- SORU03: Her ülkede kaç çalışan olduğunu sorgulayınız.

select ulke,count(id) as calısan_sayisi from personel group by ulke ;

-- SORU04: Maas ortalamasını bayanlar ve baylar olarak sorgulayınız

select cinsiyet, round(avg(maas),2) as maas_ortalama from personel group by cinsiyet;
...
select ulke,sehir,count(id) as calisan_sayisi from personel group by ulke,sehir;


-- SORU07: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını sorgulayınız.

select ulke,cinsiyet, count(*) as calisan_sayisi, round(avg(yas),2) as ortalama_yas 
from personel 
...
select ulke,cinsiyet,count(*) as calısan_sayisi,round(avg(maas))
 from personel 
 where maas>30000 
 group by ulke,cinsiyet;
...
select ulke,cinsiyet,count(*) as calısan_sayisi,round(avg(yas)) 
from personel 
...
select isim,urun_adi,sum(urun_miktari) as urun_toplamı 
from manav  
...
from manav 
group by isim,urun_adi
having urun_toplamı>=3 
order by urun_toplamı desc;

-- aggrate fonksiyon kullandıysak, group by dan sonra where  yerine having kullanacağız

-- where kullanamayız cünkü sum(urun_miktari) as urun_toplamı
-- urun_toplamı where ile kullanamayız çünkü aggregate fonksiyondan geliyor
-- yani sartin baglandigi aggregate fonksiyonu ise where yerine having olmali
...
from manav 
group by urun_adi
having max(urun_miktari) <> min(urun_mniktari)-- <  > yerine javadan aşina olduğumuz != kullanabiliriz
